                ...
                @endif
                    ...
                    <div class="panel panel-primary panel-bordered">
                        ...
                        <div class="panel-heading">
                            <h3 class="panel-title"> {{ $table }} テーブル編集</h3>
                        </div>
                        ...
                        <table id="users" class="table table-hover">
                            ...
                            @foreach($fieldOptions as $data)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <strong>種類：</strong> <span>{{ $data['type'] }}</span><br/>
                                        <strong>キー：</strong> <span>{{ $data['key'] }}</span><br/>
                                        <strong>必須：</strong>
                                        @if($data['null'] == "NO")
                                            <span>はい</span>
                                    ...
                                    <td>
                                        <input type="checkbox"
                                               ...
                                               @if(isset($dataRow->browse) && $dataRow->browse)
                                                    {{ 'checked="checked"' }}
                                               @elseif($data['key'] == 'PRI')
                                               @elseif($data['type'] == 'timestamp' && $data['field'] == 'update_date')
                                        ...
                                        <input type="checkbox"
                                               ...
                                               name="field_read_{{ $data['field'] }}" @if(isset($dataRow->read) && $dataRow->read){{ 'checked="checked"' }}@elseif($data['key'] == 'PRI')@elseif($data['type'] == 'timestamp' && $data['field'] == 'update_date')@elseif(!isset($dataRow->read)){{ 'checked="checked"' }}@endif>
                                        <label for="field_read_{{ $data['field'] }}">表示</label><br/>
                                        <input type="checkbox"
                                               id="field_edit_{{ $data['field'] }}"
                                               name="field_edit_{{ $data['field'] }}" @if(isset($dataRow->edit) && $dataRow->edit){{ 'checked="checked"' }}@elseif($data['key'] == 'PRI')@elseif($data['type'] == 'timestamp' && $data['field'] == 'update_date')@elseif(!isset($dataRow->edit)){{ 'checked="checked"' }}@endif>
                                        <label for="field_edit_{{ $data['field'] }}">編集</label><br/>
                                        <input type="checkbox"
                                               id="field_add_{{ $data['field'] }}"
                                               name="field_add_{{ $data['field'] }}" @if(isset($dataRow->add) && $dataRow->add){{ 'checked="checked"' }}@elseif($data['key'] == 'PRI')@elseif($data['type'] == 'timestamp' && $data['field'] == 'register_date')@elseif($data['type'] == 'timestamp' && $data['field'] == 'update_date')@elseif(!isset($dataRow->add)){{ 'checked="checked"' }}@endif>
                                            <label for="field_add_{{ $data['field'] }}">追加</label><br/>
                                        <input type="checkbox"
                                               id="field_delete_{{ $data['field'] }}"
                                               name="field_delete_{{ $data['field'] }}" @if(isset($dataRow->delete) && $dataRow->delete){{ 'checked="checked"' }}@elseif($data['key'] == 'PRI')@elseif($data['type'] == 'timestamp' && $data['field'] == 'update_date')@elseif(!isset($dataRow->delete)){{ 'checked="checked"' }}@endif>
                                                <label for="field_delete_{{ $data['field'] }}">削除</label><br/>
                                    </td>
                    ...
                    <div class="panel panel-primary panel-bordered">
                        ...
                        <div class="panel-heading">
                            <h3 class="panel-title">{{ ucfirst($table) }} BREAD情報</h3>
                        </div>
                        ...
                        <div class="panel-body">
                            ...
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="email">URL Slug (unique)</label>
                            ...
                            <div class="row clearfix">
                                ...
                                <div class="col-md-6 form-group">
                                    <label for="server_side">サーバ側のページング</label><br>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="description"
                                          placeholder="説明">@if(isset($dataType->description)){{ $dataType->description }}@endif</textarea>
                            </div>
            ...
            $('textarea[data-editor]').each(function () {
                ...
                textarea.closest('form').on('submit', function (ev) {
                    if (window.invalidEditors.length) {
                        ...
                        toastr.error('JSONエラー.', 'Validation errors', {"preventDuplicates": true, "preventOpenDuplicates": true});
                    } else {
