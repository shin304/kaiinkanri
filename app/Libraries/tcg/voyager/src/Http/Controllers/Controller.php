...
abstract class Controller extends BaseController
{
    ...
    public function insertUpdateData($request, $slug, $rows, $data)
    {
        ...
        foreach ($rows as $row) {
            // 2017/5/24　Tung　fix bug - Add validation required fields
            if($row->required) {
                $rules[$row->field] = 'required';
                $field_name = isset($row->display_name) ? $row->display_name : $row->field;
                $messages[$row->field. '.' . 'required'] = $field_name . 'が必須です。';
            }
    ...
    /**
     * Insert or Update with logic validations
     * @author Tung Nguyen - Added 2017/5/24
     * @param $request
     * @param $slug
     * @param $rows
     * @param $data
     ...
     * @return void
     */
    public function insertUpdateDataWithoutValidation($request, $slug, $rows, $data) {
        $rules = [];
        $messages = [];
        $multi_select = [];
        $translations = [];
