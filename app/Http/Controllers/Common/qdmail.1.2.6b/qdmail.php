...
class QdmailBase extends QdmailBranch {
    ...
    var $keep_parameter = array (
            false 
    );
    var $mta_option = null;
    var $is_create = false;
    var $address_validation_method = array (
            'this',
            'validateAddr' 
    );
    ...
    function makeDecoTemplate($deco_kind, $content) {
        ...
        foreach ( $attach as $key => $att ) {
            ...
            $end = strlen ( $ct ) - $start;
            $new_cid = $prefix . substr ( $ct, $start, $end ) . $aft;
        ...
        $this->createMail ( $this->deco_def [$this->deco_kind] ['BOUNDARY'], true );
        
        $header = '';
        foreach ( $this->deco_def [$this->deco_kind] ['TOP'] as $line ) {
            $header .= $line . $this->LFC;
        }
        $header .= 'MIME-Version: 1.0' . $this->LFC . 'Content-type: ' . key ( $this->structure [$this->deco_def [$this->deco_kind] ['STRUCTURE']] ) . '; boundary="' . $this->deco_def [$this->deco_kind] ['BOUNDARY'] . '"' . $this->LFC;
    ...
    function easy($type, $to, $subject, $content, $other_header = array(), $attach = null) {
        ...
        $to = is_string ( $to ) ? array (
                $to 
        ) : $to;
        $other_header = is_string ( $other_header ) ? array (
                'From' => $other_header 
        ) : $other_header;
        ...
        if (! isset ( $other_header_temp ['FROM'] )) {
            ...
            if (! empty ( $fromAddr )) {
                $other_header = array_merge ( $other_header, array (
                        'FROM' => array (
                                $fromAddr,
                                $fromName 
                        ) 
        ...
        $other_header = array_merge ( array (
                'TO' => $to 
        ), $other_header );
        $section = array (
                'TO' => 'to',
    ...
    function easyText($to, $subject, $content, $other_header = array(), $attach = null, $option = array()) {
        return $this->easy ( array (
                'TYPE' => 'text',
                'OPTION' => $option 
        ), $to, $subject, $content, $other_header, $attach );
    }
    ...
    function logLevel($option = null) {
        return $this->option ( array (
                __FUNCTION__ => $option 
        ), __LINE__, 0, $this->log_level_max );
    }
    function errorlogLevel($option = null) {
        return $this->option ( array (
                __FUNCTION__ => $option 
        ), __LINE__, 0, $this->errorlog_level_max );
    }
    ...
    function charsetBody($charset = null, $enc = null) {
        if (is_null ( $charset ) && is_null ( $enc )) {
            return array (
                    $this->charset_content,
                    $this->content_transfer_enc_text,
                    content_transfer_enc_html 
            );
        }
        if (is_array ( $charset )) {
    ...
    function setMbParameter($lang = null, $internal_enc = null, $detect = null) {
        if (is_array ( $lang )) {
            ...
            $this->mb_parameter_stack = array (
                    mb_language (),
                    mb_internal_encoding (),
                    $this->qd_detect_order () 
            );
    ...
    function analyzeAddr($addr, $name) {
        if (is_string ( $addr )) {
            ...
            return array (
                    array (
                            $this->tokey ['_ADDR'] => $addr,
                            $this->tokey ['_NAME'] => $name 
                    ) 
            );
        }
        // $addr is array
            ...
            if (isset ( $addr [$this->tokey ['_ADDR']] )) {
                ...
                foreach ( $addr as $ad ) {
                    ...
                    if (empty ( $_addr )) {
                        ...
                        $ret [] = array_merge ( $ad, array (
                                $this->tokey ['_ADDR'] => $_addr,
                                $this->tokey ['_NAME'] => $_name 
                        ) );
                    }
                }
                return $ret;
    ...
    function _extractNameAddr($addr) {
        ...
        return array (
                substr ( $addr, 0, strpos ( $addr, $formed_addr ) ),
                $formed_addr 
        );
    }
    function setContentArgs($type, $param) {
        ...
        return $this->{$method_name} ( $cont, isset ( $param ['_CHARSET'] ) ? $param ['_CHARSET'] : null, isset ( $param ['ENC'] ) ? $param ['ENC'] : null, isset ( $param ['_ORG_CHARSET'] ) ? $param ['_ORG_CHARSET'] : null );
    }
    // -------------------------------------------
    // Main Routine Send()
    // Option analyize
    ...
    function makeMessageId() {
        ...
        $id = 'Qdmail.' . $this->version . '_' . sha1 ( microtime () . $this->salt . mt_rand () . $req_uri ) . '@' . $right;
        return '<' . $id . '>';
    }
    ...
    function sendBase() {
        ...
        if ($fg_debug && ((0 === count ( $this->error )) && (0 === count ( $this->error_stack ))) || $this->ignore_error) {
            ...
            if ($this->smtp) {
                ...
                $fg = mail ( trim ( $this->header_for_mailfunction_to ), trim ( $this->header_for_mailfunction_subject ), $this->content_for_mailfunction, trim ( $this->header_for_mailfunction_other ) );
            } else {
                
                $fg = mail ( trim ( $this->header_for_mailfunction_to ), trim ( $this->header_for_mailfunction_subject ), $this->content_for_mailfunction, trim ( $this->header_for_mailfunction_other ), trim ( $this->mta_option ) );
            }
        ...
        $this->debugEchoLf ( $bcc, $this->content_all_for_smtp, 
                // $this->header_for_smtp,
                // $this->content_for_mailfunction,
                $this->LFC . $this->LFC, date ( 'Y-m-d H:i:s' ) );
    ...
    function createMail($boundary = null, $boundary_fix = false) {
        ...
        } elseif (($this->body_build_once && ! $this->body_already_build) || (! $this->body_build_once && (! $this->attach_build_once || ($this->attach_build_once && ! $this->attach_already_build)))) {
    ...
    function signSmime() {
        ...
        openssl_pkcs7_sign ( $temp_filename, $temp_signed_filename, $certificate, array (
                $private_key,
    ...
    function buildHeader() {
        ...
        foreach ( $this->addr_many as $section => $many ) {
            ...
            foreach ( $this->{strtolower ( $section )} as $one ) {
                $mime = $this->mimeEncode ( $one [$this->tokey ['_NAME']], isset ( $one ['_CHARSET'] ) ? $one ['_CHARSET'] : $this->charset_header, isset ( $one ['_ORG_CHARSET'] ) ? $one ['_ORG_CHARSET'] : null, strlen ( $section ) + 2 );
            ...
            if (! $many) {
                $header [$this->addr_head_name [$section]] = array (
                        array_pop ( $header [$this->addr_head_name [$section]] ) 
                );
            }
        }
        if (! empty ( $this->subject )) {
            ...
            $header ['Subject'] = $this->mimeEncode ( $subj, isset ( $this->subject ['_CHARSET'] ) ? $this->subject ['_CHARSET'] : $this->charset_header, isset ( $this->subject ['_ORG_CHARSET'] ) ? $this->subject ['_ORG_CHARSET'] : null, 9 ); // strlen(subject)+2
    ...
    function buildBody($structure, $boundary = null, $rel = false, $boundary_fix = false) {
        ...
        foreach ( $structure as $key => $value ) {
            ...
            if (is_array ( $value )) {
                ...
                $ret_header ['Content-Type'] = strtolower ( $key ) . ';' . $this->LFC . ' boundary="' . $next_boundary . '"';
                $rel = false;
    ...
    function makeContentText($content, $is_text = 'TEXT') {
        ...
        if (is_array ( $content )) {
            ...
            $target_char = isset ( $content ['_CHARSET'] ) ? $content ['_CHARSET'] : $this->charset_content;
            $length = isset ( $content ['LENGTH'] ) ? $content ['LENGTH'] : $this->wordwrap_length;
            $content_transfer_enc = ! empty ( $content ['ENC'] ) ? $content ['ENC'] : $enc;
            $content = $_content;
        } else {
    ...
    function buildAttach($one, $boundary, $inline) {
        ...
        if (isset ( $one ['CONTENT-TYPE'] )) {
            ...
            $type = isset ( $this->attach_ctype [strtolower ( $matches [1] )] ) ? $this->attach_ctype [strtolower ( $matches [1] )] : 'unkown';
        ...
        if (isset ( $this->deco_kind ) && false === $this->deco_def [$this->deco_kind] ['DISPOSITION']) {
            ...
            $disposition = $content_disposition . ';' . $this->LFC . ' filename="' . $filename . '"';
        }
        $ret_boundary = '--' . $boundary;
    ...
    function makeContentId($id) {
        if (is_null ( $this->after_id )) {
            ...
            $this->after_id = mt_rand () . '_' . str_replace ( array (
                    '@',
                    '-',
                    '/',
                    '.' 
            ), '', $fromaddr . '_' . $this->xmailer . '_' . $this->version );
        }
    ...
    function defMbSubStr($line, $start, $length, $enc) {
        return array (
                mb_substr ( $line, $start, $length, $enc ),
                $length 
        );
    }
    ...
    function logWrite($type, $message) {
        ...
        fwrite ( $fp, date ( $this->log_dateformat ) . $spacer . trim ( $message ) . $this->log_LFC );
        if ($level > 1) {
    ...
    function iniGather() {
        ...
        $ret .= $this->LFC . 'php.ini status: mb_language = ' . mb_language () . ' ; mb_internal_encoding = ' . mb_internal_encoding () . ' ; mb_detect_order = ' . implode ( ',', mb_detect_order () );
...
class QdmailComponent extends QdmailUserFunc {
    ...
    function cakeRender($content, $type, $org_charset = null, $target_charset = null) {
        ...
        $mess = null;
        $content = $view->renderElement ( $this->view_dir . DS . $type . DS . $this->template, array (
                'content' => $content 
        ), true );
        if (1.2 > ( float ) substr ( Configure::version (), 0, 3 )) {
