    ...
    #btn_return {
		...
		@if(!empty($this_screen))
			<div class="header_box box_shadow">
				<div style="float: left">
					<p>{{$lan::get('combini_request_title')}}</p>
				</div>
                        ...
                        @foreach ($file_list as $idx =>  $row)
                            <tr class="table_row">
                                ...
                                <td style="width: 60px;padding:5px 5px;text-align:center;">
                                    ...
                                    <a class="cancel_button" href="{{$_app_path}}invoice/cancelpost?invoice_year_month={{array_get($heads,'invoice_year_month')}}&file_name={{array_get($row,'processing_filename')}}">{{$lan::get('cancel_btn_title')}}</a>
                                    @endif
                                </td>
                                @endif
                            </tr>
		...
		<div id="section_content_in">
            <h4>{{$lan::get('pre_invoice_list_title')}}</h4>
                <form id="action_form" action="{{$_app_path}}invoice/ricohPostDownloadComplete" method="post">
                    {{ csrf_field() }}
                        ...
                        <div class="panel panel-default">
                            <div class="panel-heading">
                                <table class="accordion_tbl">
                                    <thead>
                                    <tr>
                                        <td style="width:5%; text-align: center;" class="text_title header">{{$lan::get('selection_title')}}</td>
...
<form action="{{$_app_path}}invoice/ricohPostProc?invoice_year_month={{array_get($heads,'invoice_year_month')}}" method="post" id="frm_return">
    {{ csrf_field() }}
</form>
@stop
