		...
		$(function() {
			...
			$(".cancel_button").click(function(e) {
			    ...
			    $( "#dialog_cancel" ).dialog({
			      title: "{{$lan::get('tranfer_title_post')}}",
			      autoOpen: false,
			      resizable: false,
			      height:140,
			      modal: true,
        ...
        @if(!empty($this_screen))
            <div class="header_box box_shadow">
                <div style="float: left">
                    <p>{{$lan::get('owner_transfer_title')}}</p>
                </div>
                        ...
                        @if($request['invoice_year_month'] == $newest_invoice_month)
                            @if($process_flg != 1 )
                                <li class="no_active" style="background:#25b4c6;">
                                    <a class="text_link" style="color:white !important;" href="{{$_app_path}}invoice/ricohPostDownload?invoice_year_month={{array_get($heads,'invoice_year_month')}}">{{$lan::get('create_data_external')}}</a>
                                </li>
                            @endif
                        @endif
                    </ul>
		...
		<div id="section_content1">
			...
			<thead>
					<tr>
						...
						<th style="width:160px;" class="text_title">{{$lan::get('status_title')}}</th>
				...
				<tbody>
					@foreach ($transfer_list as $idx => $row)
						<tr class="table_row">
							...
							{{--<td style="width:100px;padding:5px 5px;text-align:center;">{{date('Y-m-d',strtotime($row['register_date']))}}</td>--}}
							...
							<td style="width:160px;padding:5px 5px;text-align:center;">{{$request['request_status'][array_get($row,'status_flag')]}}</td>
                            {{--@if( date('Y-m-d',strtotime(array_get($row,'deadline'))) > (date('Y-m-d')))--}}
							<td style="width:60px;text-align:center;">
								@if( array_get($row, 'status_flag') < 3 )
									<a class="cancel_button"  href="{{$_app_path}}invoice/cancelpost?invoice_year_month={{array_get($heads,'invoice_year_month')}}&file_name={{array_get($row,'processing_filename')}}" style="color: #595959; text-decoration: none; font-size: 12px; border-radius: 3px; text-align: center; transition-duration: 0.1s; background-color: #f4f5f5; background-image: -webkit-linear-gradient(top, #f4f5f5, #dfdddd); padding: 3px 10px !important; width: 150px !important;">
