...
$(function() {
    ...
    $('#selectallfee, #program_fee_list').change(function() {  //on click
        
        if ( $('#selectallfee').is(":checked")) {
            var plan_id = $('#program_fee_list').val();
            $( ".program_fee_plan" ).val(plan_id);
        } else {
            // reset
            $('.program_fee_plan').prop('selectedIndex', 0);
        }
    ...
    $("#btn_edit").click(function() {
        $("#action_form").attr('action', '{{$_app_path}}program/input?id={{request('relative_id')}}');
        $("#action_form").submit();
        return false;
    });
    $("#btn_return").click(function() {
        $("#action_form").attr('action', '{{$_app_path}}program');
        $("#action_form").submit();
        return false;
    });
...
$(function() {
        $(".entry_confirm_button").click(function(e) {
            ...
            $("#dialog_join_confirm_member input[name=_program_fee_plan_id]").val($('[name=_program_fee_plan_id'+index+']').val());
...
//         // ソート条件
//         var index = $(".entry_confirm_button").index(this);
//         var select_name = '[name=_program_fee_plan_id'+index+']';
...
// ////        link = link+'&_program_fee_plan_id='+select_val;
//         link = link+'&_program_fee_plan_id='+select_val+'&sortorder='+currentSort;
...
.submit2 {
    ...
    <div id="center_content_header" class="box_border1">
            <h2 class="float_left"><i class="fa fa-bullhorn"></i>{{$lan::get('main_program_title')}}</h2>
        ...
        <div id="section_content">
            ...
            <div class="info_content padding1 box_border1">
                ...
                <tbody>
                    ...
                    @if (count(request('lesson_list')) > 0)
                    <tr>
                        <td  class="t3_2td1" colspan="4">
                        {{$lan::get('lesson_title')}}
                        </td>
                    </tr>
                    @endif
                    @foreach(request('lesson_list') as $row)
                    <tr>
                        <td class="t3_2td2">{{array_get($row, 'lesson_name')}}</td>
                        <td class="t3_2td3" colspan="3" style="text-align: left;">@if (array_get($row, 'start_date')) {{date('Y-m-d H:i',strtotime(array_get($row, 'start_date')))}} @endif</td>
                    </tr>
                    @endforeach
                    <tr>
                        <td class="t3_2td2">
...
<!-- 受講料を可変化 -->
                    ...
                    @endforeach
                    @forelse ($program_fee as $row)
                    @empty
                    <tr>
                        <td class="t3_2td2">{{$lan::get('tuition_fee')}}</td>
                        <td class="t3_2td3" colspan="3">{{$lan::get('program_fee_not_found')}}</td>
                    </tr>
                    @endforelse
                <!-- 受講料を可変化 -->
    ...
    // comment set type to student by select all students (just html, js & php code is not deleted)
                <div class="center_content_header_right">
                    ...
                    <select  style="width:300px;" name="" id="program_fee_list">
                       @if ($program_fee_plan)
                        @foreach ($program_fee_plan as $key=>$val)
        ...
        --}}
                ...
                <table class="table_list tablesorter" id="select_mail_sender">
                    ...
                    {{-- <th class="text_title header" style="width:50px;"> {{$lan::get('student_type_title')}} </th> --}}
                        {{--<th class="text_title header" style="width:80px;"> {{$lan::get('number_transmissions')}} </th>--}}
                        <th class="text_title header" style="width:100px;"> {{$lan::get('state')}} </th>
                        <th class="text_title header" style="width:80px;"> {{$lan::get('payment_status_title')}} </th>
                    {{-- <th class="text_title header" style="width:100px;"> {{$lan::get('confirmed')}} </th> --}}
                        <th class="text_title " style="width:80px;">支払方法</th>
                        <th class="text_title " style="width:20px;">定期</th>
                        <th class="text_title header" style="width:310px;">{{$lan::get('tuition_type_rates')}} </th>
                    </tr>
                    </thead>
                    <tbody>
                        ...
                        <td style="width:310px;">
                            @if ($program_fee_plan)
                                ...
                                @if ( (array_get($student,'enter') && array_get($student,'payment_selected')) || !request('editable'))
                                    @php
                                        $fee_plan = array_get($program_fee_plan,$student['plan_id']);
                                ...
                                <select  style="width:300px;" name="_program_fee_plan_id{{$loop->index}}"
                                        {{--class="@if (!array_get($student,'plan_id')) program_fee_plan @else program_fee_plan_seleted @endif">--}}
                                        class="@if (array_get($student,'delivery_count') > 0) program_fee_plan_seleted @else program_fee_plan @endif">
...
<div id="dialog_warning" class="no_title" style="display:none;">
    ...
    <div id="dialog_join_confirm_member" style="display: none;">
        <form action="{{$_app_path}}mailMessage/entry" method="">
            ...
            <input type="hidden" name="_program_fee_plan_id" value="">
            <input type="hidden" name="enable_send_mail" value="{{request('enable_send_mail')}}">
