...
<script type="text/javascript">
    $(function() {
        $("input[name='complete']").click(function() {
            if ( $("input[name='payment_method']:checked").val() == 'CRED_ZEUS' ) { // クレジットカード決済
                $("#action_form").attr('action', "{{$_app_path}}program/pay");
            }
            $("#action_form").submit();
            return false;
        });
    ...
    <div class="title_bar">
        {{$data['program_name']}} プログラム申し込み確認
    </div>
    <div class="content_header"></div>
    <div class="content_detail">
        @include('Portal.student_info'){{--学習塾情報--}}

        <form id='action_form' action="{{$_app_path}}program/complete" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
