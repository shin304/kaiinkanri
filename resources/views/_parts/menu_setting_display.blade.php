                        ...
                        @foreach ($parentMenuList as $index => $menu)
                            <li idx="{{$menu['master_menu_id']}}">
                                <input type="checkbox" class="menu_item" id="menu{{$menu['master_menu_id']}}" value="{{$menu['master_menu_id']}}"
                                       @if ($menu['default_flag'] == 1) checked @endif disabled="disabled">
                            ...
                            @if (array_key_exists($menu['master_menu_id'],$parentSubMenuList))
                                @foreach ($parentSubMenuList[$menu['master_menu_id']] as $idx => $submenu)
                                    <li idx="{{$submenu['master_menu_id']}}" class="submenu">
                                        <input type="checkbox" class="menu_item" id="menu{{$submenu['master_menu_id']}}"
                                               value="{{$submenu['master_menu_id']}}" disabled="disabled">
                                        <label for="menu{{$submenu['master_menu_id']}}">{{$lan::get($submenu['menu_name_key'])}}</label>
                                    </li>
                                @endforeach
                            @endif
                    ...
                    <ul id="view_edit">
                        @foreach ($parentMenuList as $index => $menu)
                            <li idx="{{$menu['master_menu_id']}}" class="hideLi">
                                <input type="checkbox" class="chkViewable" checked="checked" disabled="disabled">
                                <input type="checkbox" class="chkEditable" @if ($menu['master_editable'] != 1) disabled="disabled" @else checked="checked" @endif disabled="disabled">
                            </li>
                            <!-- Submenu -->
                            @if (array_key_exists($menu['master_menu_id'],$parentSubMenuList))
                                @foreach ($parentSubMenuList[$menu['master_menu_id']] as $idx => $submenu)
