                ...
                <ul id="filter-menu" class="ul-column-title" style="min-height: 200px;">
                    ...
                    @foreach(old('defaultMenuList', request('defaultMenuList')) as $dmenu)
                        ...
                        <li idx="{{$dmenu['master_menu_id']}}"  class="filter-row menu-{{$dmenu['menu_name_key']}}" >{{$lan::get($dmenu['menu_name_key'])}}
                            ...
                            @if (array_get($dmenu, 'master_editable') == 1)
                                ...
                                @else
                                    <span class="btn_auth non_editable"><input type="checkbox" name="editable_list[{{$dmenu['master_menu_id']}}]" value="{{$dmenu['master_menu_id']}}" @if (array_get($dmenu, 'editable')) checked @endif>
                                        {{--<i class="fa @if (array_get($dmenu, 'editable')) fa-check @else fa-close @endif"></i><i class="fa fa-edit"></i>--}}</span>
                                @endif
                            ...
                            {{--@if (!$dmenu['default_flag'])
                            <button type="button" class="viewable viewable-{{$dmenu['master_menu_id']}} btn_auth" title="表示">
                                <input type="checkbox" name="viewable_list[{{$dmenu['master_menu_id']}}]" value="{{$dmenu['master_menu_id']}}" @if (array_get($dmenu, 'viewable')) checked @endif>
                                <i class="fa @if (array_get($dmenu, 'viewable')) fa-check @else fa-close @endif"></i><i class="fa fa-eye"></i>
                            </button>
                            @else
                            <span class="btn_auth non_editable"><input type="checkbox" name="viewable_list[{{$dmenu['master_menu_id']}}]" value="{{$dmenu['master_menu_id']}}" @if (array_get($dmenu, 'viewable')) checked @endif>
                                <i class="fa @if (array_get($dmenu, 'viewable')) fa-check @else fa-close @endif"></i><i class="fa fa-eye"></i></span>
                            @endif--}}
                            <!--  create menu_list[] -->
                            ...
                            @if(array_key_exists($dmenu['master_menu_id'], old('defaultSubMenuList', request('defaultSubMenuList')) ))
                                <ul id="sub-reference-menu" >
                                    @foreach(old('defaultSubMenuList', request('defaultSubMenuList'))[$dmenu['master_menu_id']] as $smenu)
                                        ...
                                        @endif
                                        <!--  create view button -->
                                        {{--<button type="button" class="viewable viewable-{{$smenu['master_menu_id']}} btn_auth" title="表示">
                                            <input type="checkbox" name="viewable_list[{{$smenu['master_menu_id']}}]" value="{{$smenu['master_menu_id']}}" @if (array_get($smenu, 'viewable')) checked @endif>
...
<script type="text/javascript">
    $(function() {
       ...
       var refreshSourceList=function(){
            $("#reference-menu > li").show()
            $("#filter-menu >li").each(function(){var id=$(this).attr("idx");$("#reference-menu > li[idx="+id+"]").hide()})
        }
//        SORT LIST
