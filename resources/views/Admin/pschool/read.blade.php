                    ...
                    @foreach($dataType->readRows as $row)
                        ...
                        <div class="panel-body" style="padding-top:0;">
                            ...
                            @else
                                ...
                                @if ($row->field == 'active_flag')
                                    <p>{{ $activeFlag[$dataTypeContent->{$row->field}] }}</p>
                                @elseif ($row->field == 'zip_code')
                                    @php 
                                        $zip_code1 = substr( $dataTypeContent->{$row->field} , 0, 3); $zip_code2 = substr( $dataTypeContent->{$row->field} , 3);
                                    @endphp
                                    @if ($zip_code1 && $zip_code2)
                                    <p>〒{{$zip_code1}} - {{$zip_code2}}</p>
                                    @endif
                                @elseif ($row->field == 'business_divisions')
                                    <p>{{ $businessDivisions[$dataTypeContent->{$row->field}] }}</p>
                                @elseif ($row->field == 'country_code')
                                    <p>{{ $countryCode[$dataTypeContent->{$row->field}] }}</p>
                                @elseif ($row->field == 'login_account_id')
                                    <p>{{ $dataTypeContent->login_id }}</p>
                                @elseif ($row->field == 'pref_id')
                                    <p>{{ $dataTypeContent->pref_name }}</p>
                                @elseif ($row->field == 'city_id')
                                    <p>{{ $dataTypeContent->city_name }}</p>
                                @elseif ($row->field == 'logo')
