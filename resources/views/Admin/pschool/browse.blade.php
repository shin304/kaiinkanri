                        ...
                        <table id="dataTable" class="row table table-hover">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    @foreach($dataType->browseRows as $row)
                                        <td>
                                            ...
                                            @elseif($row->type == 'text')
                                                ...
                                                <div class="readmore" title="{{$row->field}}">
                                                @if ($row->field != 'address')
                                                    {{ strlen( $data->{$row->field} ) > 200 ? substr($data->{$row->field}, 0, 200) . ' ...' : $data->{$row->field} }}
                                                    {{--{{$data->pref_name}}　{{$data->city_name}}　{{ strlen( $data->pref_name . $data->city_name . $data->{$row->field} ) > 200 ? substr($data->{$row->field}, 0, 200) . ' ...' : $data->{$row->field} }}--}}
                                                @endif
                                                </div>
                                            ...
                                            @else
                                                ...
                                                @if ($row->field == 'group_id')
                                                    <span>{{str_pad( $data->{$row->field} ,4,"0",STR_PAD_LEFT)}}</span>
                                                @else
                                    ...
                                    <td data-active="{{$data->active_student}} "@if($data->active_student >= $data->limit_number_active*0.9) style="color: red; font-weight: bold" @endif>
                                        {{$data->limit_number_active}}
                                    </td>
