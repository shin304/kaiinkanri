                    ...
                    <div class="panel-body table-responsive">
                        <table id="dataTable" class="row table table-hover">
                            ...
                            <tbody>
                                ...
                                <!-- handle sub menu -->
                                @if (array_key_exists($data->id,$subMenuList))
                                    @foreach ($subMenuList[$data->id] as $idx => $submenu) 
                                    <tr>
                                        @foreach($dataType->browseRows as $row)
                                        <td>@include('voyager::multilingual.input-hidden-bread-browse')
                                                <div class="readmore submenu">
                                                @if ($row->field == 'menu_name_key')<i class="fa fa-angle-double-right" aria-hidden="true"></i>
                                                @endif
                                                {{ $submenu->{$row->field} }}</div></td>
                                        @endforeach
                                        <td class="no-sort no-click" id="bread-actions">
                                        @if (Voyager::can('delete_'.$dataType->name))
                                            <a href="javascript:;" title="Delete" class="btn btn-sm btn-danger pull-right delete" data-id="{{ $submenu->id }}" id="delete-{{ $submenu->id }}">
                                                <i class="voyager-trash"></i> <span class="hidden-xs hidden-sm">Delete</span>
                                            </a>
                                        @endif
                                        @if (Voyager::can('edit_'.$dataType->name))
                                            <a href="{{ route('voyager.'.$dataType->slug.'.edit', $submenu->id) }}" title="Edit" class="btn btn-sm btn-primary pull-right edit">
                                                <i class="voyager-edit"></i> <span class="hidden-xs hidden-sm">Edit</span>
                                            </a>
                                        @endif
                                        @if (Voyager::can('read_'.$dataType->name))
                                            <a href="{{ route('voyager.'.$dataType->slug.'.show', $submenu->id) }}" title="View" class="btn btn-sm btn-warning pull-right">
                                                <i class="voyager-eye"></i> <span class="hidden-xs hidden-sm">View</span>
                                            </a>
                                        @endif
                                    </td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="voyager-trash"></i>{{$lan::get('dialog_delete_message')}}</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('voyager.'.$dataType->slug.'.index') }}" id="delete_form" method="POST">
                        ...
                        <input type="submit" class="btn btn-danger pull-right delete-confirm"
                                 value="{{$lan::get('delete_ok_title')}}">
                        <button type="button" class="btn btn-default pull-right" data-dismiss="modal">{{$lan::get('cancel_title')}}</button>
                    </form>
